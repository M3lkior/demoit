...
class NavArrows extends BaseHTMLElement {
    ...
    connectedCallback() {
        ...
        //
        // reveal.forward()/back() answer true when they consumed the press on
        // a stepped slide, and navigation is skipped for that press. On a
        // slide with no steps left -- or no steps at all -- they answer false
        // and the deck moves on, so the two navigations read as one to the
        // speaker: the arrow always does "the next thing".
        document.addEventListener('keydown', event => {
            switch (event.key) {
                case 'ArrowRight':
                case 'PageDown':
...
Every element marked `reveal` -- a class on a tag, or the bare `reveal` flag
on a :::col, which renders to the same class -- is one step. The arrows walk
those steps before they walk the deck. Marking an element is the whole opt-in:
a slide with none behaves exactly as it did before.

Steps are read in document order, which is the order they are written in, so
an author reorders the reveal by moving the markup and nothing else.

State lives in the page and not in the URL: each slide is its own document
here, so coming back to a slide starts it over at step zero. That is what a
speaker wants when they step back to re-explain something, and it costs
nothing to leave.
*/
const reveal = (() => {
    const steps = Array.from(document.querySelectorAll('.reveal'));
    let shown = 0;

    function forward() {
        if (shown >= steps.length) return false;
        steps[shown++].classList.add('is-revealed');
        return true;
    }

    function back() {
        if (shown === 0) return false;
        steps[--shown].classList.remove('is-revealed');
        return true;
    }

    // A click anywhere on the slide advances it too, for a speaker holding a
    // presenter remote that sends a click rather than a key. Clicks on the
    // live panes are left alone: a terminal, an embedded browser and the code
    // viewer are things one clicks *into*, and swallowing that to advance a
    // slide would make a demo unusable.
    if (steps.length) {
        document.addEventListener('click', event => {
            if (event.target.closest('web-term, web-browser, vs-code, source-code, fake-window, a, button')) return;
            forward();
        });
    }

    return {forward, back, count: () => steps.length};
})();
