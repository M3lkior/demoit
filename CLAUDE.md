...
**`demoit --dev <talk>` starts the talk's watcher itself** (`watchCSS` in `main.go`), so during a writing session the two commands above are one. It is a silent no-op when `hack/css.sh` is not next to the working directory — a `go install`ed binary has no repository around it — or when the folder carries no `tailwind.src.css`. The watcher stops with the server, on Ctrl+C and on a signal alike. Note that the script passes **`--watch=always`**, not `--watch`: the Tailwind CLI stops watching as soon as stdin is closed, which is what it sees when anything but a terminal starts it, and it would otherwise print "building …", exit, and leave a watch that never rebuilds.
...
| `source` | Shown as a "Source: …" line under the slide. Single reference, the original key. |
| `sources` | A YAML list of references, rendered by the `source` partial as `<ul class="sources">` under the slide, **at most `deck.MaxSources` (5) entries** — one more and the slide renders as an error naming the count, rather than silently dropping a reference. A slide may declare both keys; both render. The engine only emits the list: `sdd-talk/.demoit/style.css` is what lays it out on a single line with middle-dot separators, since five stacked entries would eat the foot of a 16:9 slide. Entries are plain text, HTML-escaped like any other frontmatter string, so an `<a href>` would show as markup, not a link — write the bare URL. |
...
**An attribute may be a bare flag.** `:::col{class="col-span-4 panel" reveal}` — a key with no `=` records its presence, and a renderer tests for the key rather than for what it holds. This is what `reveal` uses, and the reason a flag is written bare rather than as `reveal=true`: a value that can only ever be one thing is a value to keep typing. Note the trade: `::term{path}` no longer reports an error, it renders a `<web-term>` with no path.

**Progressive reveal: `reveal` marks one step of a slide.** Write it as a class on a tag (`<p class="reveal">`) or as the bare flag on a `:::grid` / `:::col`, which renders to that same class so a whole block reveals as one. There is **no key to set on the slide** — marking an element is the entire opt-in, and a slide with no marked element behaves exactly as before. Steps are hidden at first and shown one at a time by ArrowRight / PageDown / Space (or a click anywhere outside a live pane: a terminal, an embedded browser, the code viewer and a link keep their own clicks), rolled back by ArrowLeft / PageUp, and only once the last one is out does the deck move to the next slide. Steps go in document order, so reordering the reveal means moving the markup. `visibility` is hidden alongside `opacity` so the grid does not reflow as steps arrive — otherwise every panel would jump sideways as its neighbours land. State is per page load: coming back to a slide starts it at step zero, which is what a speaker stepping back to re-explain wants. **The walk itself lives in the talk's `demoit.js`** (in `NavArrows` and the `reveal` module beside it), so a talk whose copy predates this shows every step at once — `sdd-talk/` has it, `impact-framework/` and `sample/` do not.
...
**Other routes:** `/grid` renders every step as an iframe (also warms the browser cache so slide transitions don't lag); `/pdf` drives headless Chrome via chromedp at 1920x1080@2x, 4 parallel tabs, into a gofpdf document. It loads each slide with `?export=pdf`, which the index template turns into `data-export="pdf"` on `<html>`: the engine sheet then shows every `reveal` step and hides the GitHub star button, and buttons.js is not loaded. It sizes the viewport before navigating and waits (10s at most) for every `.mermaid` to hold its SVG with `renderMermaid`'s counter-scale removed, since a capture taken on the load event printed an empty pane. Chrome events the vendored cdproto is too old to decode are dropped from the log rather than printed as `ERROR: could not unmarshal event`; `/qrcode?url=` returns a PNG; `/last` redirects to the final slide.
...
**The em dash is forbidden in slide prose**, as the `&mdash;` entity and as the literal `—` character alike. Write a colon instead, with the French spacing: a space on both sides. Where the clause is a coordination rather than an explicitation (`A — et B`, `Name — Company`), a comma reads better than a colon, so use one: the ban is on the em dash, not a licence to punctuate badly. This covers slide bodies and `speakernotes:` alike, in every talk — including prose pasted in from a brief, which is where the literal character usually sneaks back in. `sdd-talk/demoit.md` is written while its speaker edits it in his own IDE, so a conversion pass there does not stay done: an editor buffer saved from a pre-conversion state brings the em dashes straight back. Check with `grep -c '&mdash;\|—'` rather than trusting that an earlier pass still holds, and re-run the pass at the end, not in the middle of someone's editing session. `impact-framework/` and `sample/` carry neither form.

**No GFM: a Markdown table renders as a paragraph of pipes.** `deck/deck.go:247` builds goldmark with `directive.Extension` and nothing else, so none of the GitHub-flavoured extensions are on — no tables, no strikethrough, no autolinks, no task lists. Write a table as raw HTML (`<table class="border xlarge-text">`, styled by beercss); `sdd-talk/demoit.md`'s S12, S16 and S23 are the worked examples. Note the flip side, which bites more often: **Markdown emphasis does not work inside a raw HTML block.** Goldmark stops parsing inline Markdown as soon as a block opens with a tag, so `*mot*` inside a `<p>`, `<li>` or `<h5>` reaches the browser as literal asterisks — write `<em>mot</em>` there. `*mot*` only works in a Markdown paragraph or heading. In `sdd-talk/`, `<em>` renders as non-italic blue and is the deck's only emphasis mechanism, so it is spent sparingly, one or two per slide.

**Slide writing goes through the `speaker-minion` agent**, defined in `.claude/agents/speaker-minion.md` (checked in, so it is available to every session on this repo). Prose for a deck (slide bodies, speaker notes) is written by delegating to it rather than by writing the Markdown directly, so that a single voice and slide grammar hold across a talk. Its own rules matter to whoever briefs it: slides stay terse so the speaker can improvise around them rather than read them, the tone is educational and plainly human, and every slide's sources go in its speaker notes with two or three independent references corroborating each claim. It has no web tools, so a claim needing verification is checked before delegating and handed to it in the brief.
